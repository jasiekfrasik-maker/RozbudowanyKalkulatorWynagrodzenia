window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('uop_calc').addEventListener('click', ()=>{
    const tryb = document.querySelector('input[name="uop_tryb"]:checked').value;
    ...
    if(tryb==='godz'){ stawkaH = stawkaKwota; kwotaBruttoUmowy = godzStandard*stawkaH; }
    else { kwotaBruttoUmowy = stawkaKwota; stawkaH = godzStandard>0 ? kwotaBruttoUmowy/godzStandard : 0; }
    ...
    document.getElementById('uop_potr').innerHTML = [
    ...
    document.getElementById('uop_pods').innerHTML = [
  ...
  document.getElementById('z_calc').addEventListener('click', ()=>{
    ...
    if(student){
      ...
      set('z_netto', PLN(brutto)); return;
    }

    const podstawaZUS = brutto;
    const em = podstawaZUS*SKLAD
